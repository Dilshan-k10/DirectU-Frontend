...
const NAV_ITEMS = [
  ...
  },
  {
    ...
    path: '/admin/users',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 hidden">
...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed top-0 left-0 z-30 h-full w-64 bg-brand-card flex flex-col shrink-0
        ...
        <nav className="flex-1 px-3 py-5 space-y-1 overflow-y-auto">
          {NAV_ITEMS.map(({ label, path, icon }) => (
            <NavLink
              ...
              onClick={onClose}
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-5 rounded-xl text-sm font-medium transition-all duration-200
                ${isActive
