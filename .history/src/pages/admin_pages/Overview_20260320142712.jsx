...
const Overview = () => {
  ...
  useEffect(() => {
    getQualifiedApplications()
      .then((res) => {
        ...
        setTotalActiveDegrees(res.count);
      })
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-x-8 ga">
        {CARDS.map((card) => (
          <StatCard
            key={card.label}
            {...card}
            ...
            loading={
              (card.label === "Total Applications" || card.label === "Total Active Degrees" || card.label === "Total Applicants" || card.label === "Active Intake") && loading
            }
          />
        ))}
      </div>
