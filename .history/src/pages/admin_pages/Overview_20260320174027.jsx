    ...
    <div>
      ...
      <div className="flex col-2 gap-6">
        <div className="mt-10 bg-brand-light rounded-2xl p-8 flex-1">
          <h2 className="text-gray-700 font-semibold text-base mb-1">Application Status Breakdown</h2>
          <p className="text-gray-400 text-xs mb-6">Distribution of all applications by current status</p>
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie
                data={[
                  { name: "Qualified",     value: totalQualifiedApplications    || 0 },
                  { name: "Not Qualified", value: totalNotQualifiedApplications || 0 },
                  { name: "Submitted",     value: (totalApplications || 0) - (totalQualifiedApplications || 0) - (totalNotQualifiedApplications || 0) - (totalPendingTestApplications || 0) - (totalRejectedApplications || 0) },
                  { name: "Pending Test",  value: totalPendingTestApplications  || 0 },
                ...
                dataKey="value"
              >
                {["#6366f1", "#f97316", "#3b82f6", "#eab308", "#ef4444"].map((color, i) => (
                  <Cell key={i} fill={color} />
                ))}
              </Pie>
              <Tooltip
                contentStyle={{ backgroundColor: "#1a1f36", border: "none", borderRadius: "12px", color: "#fff" }}
        ...
        <div className="mt-10 bg-brand-light rounded-2xl p-8 flex-1">
          <h2 className="text-gray-700 font-semibold text-base mb-1">Applications vs Qualified</h2>
          <p className="text-gray-400 text-xs mb-6">Total applications compared to qualified count</p>
          <ResponsiveContainer width="100%" height={260}>
            <BarChart
              data={[
                { name: "Total",     count: totalApplications            || 0 },
                { name: "Qualified", count: totalQualifiedApplications   || 0 },
                { name: "Submitted", count: (totalApplications || 0) - (totalQualifiedApplications || 0) - (totalNotQualifiedApplications || 0) - (totalPendingTestApplications || 0) - (totalRejectedApplications || 0) },
                { name: "Rejected",  count: totalRejectedApplications    || 0 },
              ]}
              barSize={36}
            >
              <CartesianGrid strokeDasharray="3 3" stroke="#2d3555" vertical={false} />
              ...
              <Bar dataKey="count" radius={[6, 6, 0, 0]}>
                {["#6366f1", "#22c55e", "#3b82f6", "#ef4444"].map((color, i) => (
                  <Cell key={i} fill={color} />
                ))}
              </Bar>
            </BarChart>
