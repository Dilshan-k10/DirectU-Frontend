...
// get number of applications for a specific program: Fetches the count of applications for a given program ID.
export const getApplicationCountByProgram = async () => {
  try {
    const response = await axiosClient.get("/applications/all/applications");
    const grouped = {};
    response.data.data.forEach((app) => {
      const name = app.program?.name || app.programId;
      grouped[name] = (grouped[name] || 0) + 1;
    });
    return Object.entries(grouped).map(([name, count]) => ({ name, count }));
  } catch (error) {
    console.error("Error fetching application count by program:", error);
    throw error;
  }
};
...
export const getApplicantExamDetails = async (applicationId) => {
  if (!applicationId) throw new Error('Missing applicationId');
  const response = await axiosClient.get(`/uniadmin/applicants/${applicationId}/exam`);
  return response.data;
};
...
export const getApplicantanalysisResultById = async () => { 
  try {
    const response = await axiosClient.get(`/analysis/applications`, { responseType: 'json' });
    return { data: response.data };
